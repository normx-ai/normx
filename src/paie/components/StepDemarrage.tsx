...
const ANNEES: number[] = [2024, 2025, 2026, 2027];

interface StepDemarrageProps {
  mois: number;
  annee: number;
...
function StepDemarrage({ mois, annee, onMoisChange, onAnneeChange }: StepDemarrageProps) {
  return (
      ...
      <div className="step-periode-section">
        ...
        <div className="step-periode-selects">
          ...
          <div className="wizard-form-group">
            ...
            <select value={mois} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => onMoisChange(Number(e.target.value))}>
              {MOIS.map((m, i) => <option key={i} value={i}>{m}</option>)}
            </select>
          </div>
          <div className="wizard-form-group">
            <label>Année <span className="required">*</span></label>
            <select value={annee} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => onAnneeChange(Number(e.target.value))}>
