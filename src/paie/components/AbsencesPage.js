...
import { ABSENCES_CONGES_PAYEES, JOURS_SEMAINE, DEFAULT_PLANNING_SALARIE } from '../data/salarieData';

function AbsencesPage({ salaries = [] }) {
  const [absences, setAbsences] = useState([]);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ salarie_id: '', type: '', date_debut: '', date_fin: '', jours: '', motif: '' });
        ...
        <div style={{ padding: 16, background: '#fafafa', borderRadius: 8, border: '1px solid #e5e5e5', marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              ...
              <select value={form.salarie_id} onChange={e => setForm(p => ({ ...p, salarie_id: e.target.value }))} style={inputStyle}>
            ...
            <div>
              ...
              <input type="number" value={form.jours} onChange={e => setForm(p => ({ ...p, jours: e.target.value }))} placeholder="Ex: 5" style={inputStyle} />
            </div>
            <div>
              <label style={labelStyle}>Motif</label>
              <input type="text" value={form.motif} onChange={e => setForm(p => ({ ...p, motif: e.target.value }))} placeholder="Motif (optionnel)" style={inputStyle} />
            </div>
          </div>
...
const labelStyle = { display: 'block', fontSize: 12, fontWeight: 600, color: '#333', marginBottom: 4 };
const inputStyle = { width: '100%', padding: '8px 10px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 13 };
const thStyle = { padding: '10px 12px', textAlign: 'left', fontSize: 12, fontWeight: 600 };
