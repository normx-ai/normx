...
export const CONVENTIONS_RUBRIQUES: Record<string, ConventionCollective> = {
 ...
 */
export function getRubriquesConvention(conventionCode: string): ConventionCollective {
 ...
 */
export function getPrimesConvention(conventionCode: string): RubriqueConvention[] {
 ...
 */
export function getIndemnitésConvention(conventionCode: string): RubriqueConvention[] {
  const conv = getRubriquesConvention(conventionCode);
 ...
 */
export function getMajorationsConvention(conventionCode: string): RubriqueConvention[] {
  const conv = getRubriquesConvention(conventionCode);
 ...
 */
export function calculerPrimeAnciennete(conventionCode: string, anneesAnciennete: number, salaireBase: number): number {
  const conv = getRubriquesConvention(conventionCode);
 ...
 */
export function calculerHeuresSupp(conventionCode: string, heuresTravaillees: number, tauxHoraire: number, estDimancheFerie: boolean = false, estNuit: boolean = false): number {
  const conv = getRubriquesConvention(conventionCode);
  ...
  for (const tranche of hs.filter(h => !h.type)) {
    const debut = (tranche.de ?? 41) - 41;
    const fin = (tranche.a ?? 999) - 40;
 ...
 */
export function getBaremeLicenciement(conventionCode: string): LicenciementTranche[] {
  const conv = getRubriquesConvention(conventionCode);
 ...
 */
export function getResumeConvention(conventionCode: string): {
  label: string;
  ancienneteMax: number;
  ancienneteDebut: number;
  nbPrimes: number;
  nbIndemnités: number;
  nbMajorations: number;
