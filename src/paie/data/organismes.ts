...
interface OrganismeBrancheData {
  code: string;
  nom: string;
  description: string;
...
interface ImpotData {
  ...
  bareme?: { min: number; max: number | null; taux: number; forfait?: number }[];
  frais_professionnels?: number;
  taux_non_resident?: number;
  taux_resident?: number;
  repartition?: Record<string, number>;
  note?: string;
  centre_ville?: number;
  peripherie?: number;
  montant_annuel?: number;
  mois_prelevement?: string;
}
...
interface OrganismeData {
  ...
  sigle: string;
  type: string;
  branches?: Record<string, OrganismeBrancheData>;
  totaux?: { taux_patronal_total: number; taux_salarial_total: number };
  impots?: Record<string, ImpotData>;
}

interface OrganismesMap {
  cnss: OrganismeData;
  camu: OrganismeData;
  direction_impots: OrganismeData;
}

interface ResumeLigne {
  label: string;
  patronal: string;
  salarial: string;
  plafond: string;
...
interface ResumeOrganisme {
  organisme: string;
  nom: string;
  lignes: ResumeLigne[];
}

interface OrganismeDeclaration {
  code: string;
  nom: string;
  declarations: string[];
  periodicite: string;
...
const ORGANISMES: OrganismesMap = {
  cnss: {
    code: 'CNSS',
 ...
 */
export function getResumeOrganismes(): ResumeOrganisme[] {
  return [
    {
      organisme: 'CNSS',
 ...
 */
export function getOrganismesDeclarations(): OrganismeDeclaration[] {
  return [
    {
      code: 'CNSS',
