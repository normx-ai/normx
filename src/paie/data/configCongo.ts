...
interface ConfigCongoTaux {
  label: string;
  valeur: string;
  plafond: string;
}

interface ConfigCongoOrganisme {
  key: string;
  label: string;
  description: string;
  champs: ConfigCongoChamp[];
  taux: ConfigCongoTaux[];
}

interface ConfigCongoTauxLigne {
  element: string;
  valeur: string;
  unite: string;
  plafond?: string;
...
interface ConfigCongoTauxSection {
  key: string;
  label: string;
  type: string;
  lignes: ConfigCongoTauxLigne[];
}

interface ConfigCongoType {
  organismes: ConfigCongoOrganisme[];
  parametresSections: { key: string; label: string; component: string }[];
  tauxSections: ConfigCongoTauxSection[];
  planningDefaults: { heuresJour: string; heuresSemaine: number; heuresMois: number };
  steps: string[];
  identifiantLabel: string;
  identifiantPlaceholder: string;
}

const CONFIG_CONGO: ConfigCongoType = {
  organismes: [
    {
