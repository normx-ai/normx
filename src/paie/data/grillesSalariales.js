...
export const GRILLES_SALARIALES = {
 ...
 */
export function getGrille(conventionCode) {
 ...
 */
export function getCategoriesGrille(conventionCode) {
 ...
 */
export function getEchelonsGrille(conventionCode, categorie) {
  const grille = getGrille(conventionCode);
  if (!grille) return [];
 ...
 */
export function getSalaireBase(conventionCode, categorie, echelon) {
  const grille = getGrille(conventionCode);
  if (!grille) return null;
  const cat = grille.categories.find(c => String(c.cat) === String(categorie));
  if (!cat) return null;
  const idx = parseInt(echelon, 10) - 1;
