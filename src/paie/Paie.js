...
function Paie({ cabinetId }) {
  ...
  const [showAssistantDeclarations, setShowAssistantDeclarations] = useState(false);
  const [lastCreatedSalarie, setLastCreatedSalarie] = useState(null);
  const [paieMode, setPaieMode] = useState('wizard');
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (etabData.etablissements) {
        setEtablissements(etabData.etablissements.map(e => ({
          ...e,
      ...
      if (salData.salaries) {
        setSalaries(salData.salaries.map(s => ({
          id: s.id,
      ...
      console.error('Erreur chargement données paie:', err);
      setConfigLoaded(true);
    }
  }, [cabinetId]);
  ...
  const saveConfig = useCallback(async () => {
    ...
    try {
      ...
      console.error('Erreur sauvegarde config paie:', err);
    }
  ...
  const handleAddEtablissement = async (etab) => {
    if (!cabinetId) {
    ...
    try {
      ...
      if (result.etablissement) {
        const saved = result.etablissement;
  ...
  const handleAddSalarie = async (data) => {
    const etabId = data.emploi?.etablissement || null;

    if (!cabinetId) {
      const newSalarie = { ...data, id: Date.now() };
                    ...
                    <tr className="etab-search-row">
                      <th>
                        <div className="etab-search-cell">
                          <input type="text" value={searchNom} onChange={e => setSearchNom(e.target.value)} placeholder="" />
                          <span className="etab-search-icon"><Icon name="search" size="sm" /></span>
                        </div>
                      </th>
                      <th>
                        <div className="etab-search-cell">
                          <input type="text" value={searchPrenom} onChange={e => setSearchPrenom(e.target.value)} placeholder="" />
                          <span className="etab-search-icon"><Icon name="search" size="sm" /></span>
                        </div>
                      </th>
                      <th></th>
