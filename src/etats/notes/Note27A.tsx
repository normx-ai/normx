...
function Note27A({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note27AProps): React.JSX.Element {
  ...
  useEffect(() => { if (!entiteId) return; fetch('/api/entites/' + entiteId).then(r => r.json()).then(ent => { const d = ent.data || {}; setParams(d); setCommentaire(d['note27a_commentaire'] || DEFAULT_COMMENTAIRE); if (d['note27a_adjustments']) { try { setAdjustments(JSON.parse(d['note27a_adjustments'])); } catch { /* */ } } }).catch(() => {}); }, [entiteId]);
  ...
  const handleSave = async () => { setSaving(true); try { const d: Record<string, string> = { ...params, note27a_adjustments: JSON.stringify(adjustments), note27a_commentaire: commentaire }; const r = await fetch(`/api/entites/${entiteId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data: d }) }); if (r.ok) { setParams(d); setSaved(true); setEditing(false); setTimeout(() => setSaved(false), 3000); } } catch { /* */ } setSaving(false); };
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const u = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = u; a.download = 'Note27A_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(u); };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 27A — Charges de personnel</div>
