...
function Note6({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note6Props): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch('/api/entites/' + entiteId)
      ...
      .then(ent => {
        const data = ent.data || {};
        setParams(data);
        setCommentaire(data['note6_commentaire'] || DEFAULT_COMMENTAIRE);
        if (data['note6_adjustments']) {
          try { setAdjustments(JSON.parse(data['note6_adjustments'])); } catch { /* */ }
        }
      })
      .catch(() => {});
  }, [entiteId]);
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const url = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = url; a.download = 'Note6_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(url); };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 6 — Stocks et en cours</div>
      ...
      {previewUrl && (
        <div className="etat-preview-overlay" onClick={closePreview}>
          <div className="etat-preview-modal" onClick={e => e.stopPropagation()}>
            ...
            <iframe src={previewUrl} className="etat-preview-iframe" title="Aperçu Note 6" />
          </div>
        </div>
      )}
      ...
      <div ref={pageRef} style={{
        ...
        <p style={{ fontSize: 11, margin: '10px 0 6px', fontStyle: 'italic', color: '#444' }}>
          (1) Les stocks H.A.O. seront inscrits dans l'actif circulant H.A.O. que lorsque leur montant total est significatif (supérieur à 5 % du total de l'actif circulant).
        </p>
