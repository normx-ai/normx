...
const RUBRIQUES: Rubrique[] = [
  { label: 'Pertes sur créances clients', prefixes: ['651'] },
  { label: 'Pertes sur autres débiteurs', prefixes: ['652'] },
  { label: 'Quote-part de résultat sur opérations faites en commun', prefixes: ['653'] },
  { label: 'Valeur comptable des cessions courantes d\'immobilisations', prefixes: ['654'] },
  { label: 'Indemnités de fonction et autres rémunérations d\'administrateurs', prefixes: ['655'] },
  { label: 'Dons et mécénat', prefixes: ['656'] },
  { label: 'Autres charges diverses', prefixes: ['657', '658'] },
  { label: 'Charges pour provisions et provisions pour risques à court terme d\'exploitation (voir note 28)', prefixes: ['659'] },
];

function Note26({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note26Props): React.JSX.Element {
  ...
  useEffect(() => { if (!entiteId) return; fetch('/api/entites/' + entiteId).then(r => r.json()).then(ent => { const d = ent.data || {}; setParams(d); setCommentaire(d['note26_commentaire'] || DEFAULT_COMMENTAIRE); if (d['note26_adjustments']) { try { setAdjustments(JSON.parse(d['note26_adjustments'])); } catch { /* */ } } }).catch(() => {}); }, [entiteId]);
  ...
  const handleSave = async () => { setSaving(true); try { const d: Record<string, string> = { ...params, note26_adjustments: JSON.stringify(adjustments), note26_commentaire: commentaire }; const r = await fetch(`/api/entites/${entiteId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data: d }) }); if (r.ok) { setParams(d); setSaved(true); setEditing(false); setTimeout(() => setSaved(false), 3000); } } catch { /* */ } setSaving(false); };
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const u = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = u; a.download = 'Note26_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(u); };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 26 — Autres charges</div>
