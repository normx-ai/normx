...
function Note26({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note26Props): React.JSX.Element {
  ...
  useEffect(() => {
    if (!params['note26_commentaire'] && !params['note26_adjustments']) return;
    setCommentaire(params['note26_commentaire'] || DEFAULT_COMMENTAIRE);
    if (params['note26_adjustments']) {
      try { setAdjustments(JSON.parse(params['note26_adjustments'])); } catch { /* */ }
    }
  }, [params]);
  ...
  const handleSave = () => saveParams({
    ...params,
    note26_adjustments: JSON.stringify(adjustments),
    note26_commentaire: commentaire,
  });
    ...
    <div>
      ...
      <div style={{ margin: '12px 20px', padding: '12px 16px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: 8, fontSize: 12, color: '#1e40af', lineHeight: 1.6 }}>
        <div style={{ fontWeight: 700, marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
          <LuInfo size={14} /> Note d'information — Note 26
        </div>
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          <li><strong>Charges diverses :</strong> Compte 65 — pertes sur creances irrecouvrables, jetons de presence, redevances.</li>
          <li><strong>Quotes-parts communes :</strong> Quote-part de resultat sur operations faites en commun (compte 658).</li>
          <li><strong>Valeurs comptables cedees :</strong> Pour les elements d'actif cedes dans le cadre des operations ordinaires.</li>
          <li>Ne pas confondre avec les charges HAO (Note 30).</li>
        </ul>
      </div>
