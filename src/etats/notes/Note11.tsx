...
const RUBRIQUES_BRUT: RubriqueDispo[] = [
  ...
  { label: 'Banques, dépôt à terme', prefixes: ['523'] },
  { label: 'Autres Banques', prefixes: ['524', '525', '526', '527', '528'] },
  { label: 'Banques intérêts courus', prefixes: ['529'] },
  { label: 'Chèques postaux', prefixes: ['531'] },
  { label: 'Autres établissement financiers', prefixes: ['532', '533', '534', '535', '536', '537', '538'] },
  { label: 'Etablissement financiers intérêts courus', prefixes: ['539'] },
  { label: 'Instruments de trésorerie', prefixes: ['54'] },
  { label: 'Caisse', prefixes: ['571'] },
  { label: 'Caisse électronique mobile', prefixes: ['572'] },
  { label: 'Régies d\'avances et virements accréditifs', prefixes: ['581', '582'] },
];
...
function Note11({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note11Props): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch('/api/entites/' + entiteId)
      ...
      .then(ent => {
        const data = ent.data || {};
        setParams(data);
        setCommentaire(data['note11_commentaire'] || DEFAULT_COMMENTAIRE);
        if (data['note11_adjustments']) {
          try { setAdjustments(JSON.parse(data['note11_adjustments'])); } catch { /* */ }
        }
      })
      .catch(() => {});
  }, [entiteId]);
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const url = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = url; a.download = 'Note11_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(url); };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 11 — Disponibilités</div>
      ...
      {previewUrl && (
        <div className="etat-preview-overlay" onClick={closePreview}>
          <div className="etat-preview-modal" onClick={e => e.stopPropagation()}>
            ...
            <iframe src={previewUrl} className="etat-preview-iframe" title="Aperçu Note 11" />
          </div>
        </div>
      )}
