...
function Note24({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note24Props): React.JSX.Element {
  ...
  const pdf = usePDFPreview({ pageRef, fileName: `Note24_${annee}.pdf`, editing, setEditing });
  ...
  useEffect(() => {
    if (!params['note24_commentaire'] && !params['note24_adjustments']) return;
    setCommentaire(params['note24_commentaire'] || DEFAULT_COMMENTAIRE);
    if (params['note24_adjustments']) {
      try { setAdjustments(JSON.parse(params['note24_adjustments'])); } catch { /* */ }
    }
  }, [params]);
  ...
  const handleSave = () => saveParams({
    ...params,
    note24_adjustments: JSON.stringify(adjustments),
    note24_commentaire: commentaire,
  });
  ...
  return (
    <div>
      <NoteToolbar
        title="Note 24 — Services exterieurs"
      ...
      {pdf.previewUrl && (
        <PDFPreviewModal previewUrl={pdf.previewUrl} title="Apercu — Note 24" onClose={pdf.closePreview} onDownload={pdf.downloadPDF} onPrint={pdf.printPDF} />
      )}
