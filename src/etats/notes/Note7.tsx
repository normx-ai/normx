...
function Note7({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note7Props): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch('/api/entites/' + entiteId)
      ...
      .then(ent => {
        const data = ent.data || {};
        setParams(data);
        setCommentaire(data['note7_commentaire'] || DEFAULT_COMMENTAIRE);
        if (data['note7_adjustments']) {
          try { setAdjustments(JSON.parse(data['note7_adjustments'])); } catch { /* */ }
        }
      })
      .catch(() => {});
  }, [entiteId]);
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const url = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = url; a.download = 'Note7_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(url); };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 7 — Clients</div>
      ...
      {previewUrl && (
        <div className="etat-preview-overlay" onClick={closePreview}>
          <div className="etat-preview-modal" onClick={e => e.stopPropagation()}>
            ...
            <iframe src={previewUrl} className="etat-preview-iframe" title="Aperçu Note 7" />
          </div>
        </div>
      )}
      ...
      <div ref={pageRef} style={{
        ...
        <div className="etat-header-officiel">
          ...
          <div className="etat-sub-titre">NOTE 7<br />CLIENTS</div>
        </div>
