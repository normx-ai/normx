...
function Note23({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note23Props): React.JSX.Element {
  ...
  useEffect(() => { if (!entiteId) return; fetch('/api/entites/' + entiteId).then(r => r.json()).then(ent => { const data = ent.data || {}; setParams(data); setCommentaire(data['note23_commentaire'] || DEFAULT_COMMENTAIRE); if (data['note23_adjustments']) { try { setAdjustments(JSON.parse(data['note23_adjustments'])); } catch { /* */ } } }).catch(() => {}); }, [entiteId]);
  ...
  const handleSave = async () => { setSaving(true); try { const data: Record<string, string> = { ...params, note23_adjustments: JSON.stringify(adjustments), note23_commentaire: commentaire }; const res = await fetch(`/api/entites/${entiteId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data }) }); if (res.ok) { setParams(data); setSaved(true); setEditing(false); setTimeout(() => setSaved(false), 3000); } } catch { /* */ } setSaving(false); };
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const url = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = url; a.download = 'Note23_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(url); };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 23 — Transports</div>
      ...
      <div ref={pageRef} style={{ width: '210mm', minHeight: '297mm', background: '#fff', margin: '0 auto 20px', padding: '8mm 10mm', boxShadow: '0 2px 12px rgba(0,0,0,0.1)', boxSizing: 'border-box', fontFamily: "'Outfit', 'Segoe UI', Arial, sans-serif", fontSize: 12, color: '#1a1a1a' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 0 }}>
          ...
          <tbody>
            {rows.filter(r => !hideEmpty || r.vals.anneeN !== 0 || r.vals.anneeN1 !== 0).map(r => (<tr key={r.label}><td style={tdStyle}>{r.label}</td><td style={tdRight}>{renderAdjInput(r.label, 'anneeN', r.vals.anneeN)}</td><td style={tdRight}>{renderAdjInput(r.label, 'anneeN1', r.vals.anneeN1)}</td><td style={{ ...tdRight, background: '#fafafa' }}>{r.vals.variation !== 0 ? r.vals.variation.toFixed(1) + ' %' : ''}</td></tr>))}
