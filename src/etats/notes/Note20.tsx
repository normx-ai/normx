...
function Note20({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note20Props): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch('/api/entites/' + entiteId)
      ...
      .then(ent => {
        const data = ent.data || {};
        setParams(data);
        setCommentaire(data['note20_commentaire'] || DEFAULT_COMMENTAIRE);
        if (data['note20_adjustments']) { try { setAdjustments(JSON.parse(data['note20_adjustments'])); } catch { /* */ } }
      })
      .catch(() => {});
  }, [entiteId]);
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const data: Record<string, string> = { ...params, note20_adjustments: JSON.stringify(adjustments), note20_commentaire: commentaire };
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const url = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = url; a.download = 'Note20_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(url); };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 20 — Banques, crédit d'escompte et de trésorerie</div>
      ...
      {previewUrl && (
        <div className="etat-preview-overlay" onClick={closePreview}>
          <div className="etat-preview-modal" onClick={e => e.stopPropagation()}>
            ...
            <iframe src={previewUrl} className="etat-preview-iframe" title="Aperçu Note 20" />
          </div>
        </div>
      )}
