...
const RUBRIQUES: Rubrique[] = [
  { label: 'Titres participatifs', prefixes: ['161'] },
  { label: 'Avances conditionnées', prefixes: ['162'] },
  { label: 'Titres subordonnés à durée indéterminée (T.S.D.I.)', prefixes: ['163'] },
  { label: 'Obligations remboursables en actions (O.R.A.)', prefixes: ['164'] },
  { label: 'Autres', prefixes: ['165', '166', '167', '168', '169'] },
];
...
function Note15B({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note15BProps): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch('/api/entites/' + entiteId)
      ...
      .then(ent => {
        const data = ent.data || {};
        setParams(data);
        setCommentaire(data['note15b_commentaire'] || DEFAULT_COMMENTAIRE);
        if (data['note15b_adjustments']) {
          try { setAdjustments(JSON.parse(data['note15b_adjustments'])); } catch { /* */ }
        }
      })
      .catch(() => {});
  }, [entiteId]);
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const url = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = url; a.download = 'Note15B_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(url); };
  const printPDF = () => { if (!previewUrl) return; const w = window.open(previewUrl); if (w) { w.onload = () => w.print(); } };

  const thStyle: React.CSSProperties = { border: '0.5px solid #000', padding: '6px 8px', fontSize: 12, fontWeight: 700, textAlign: 'center', verticalAlign: 'middle', background: '#f5f5f5' };
  const tdStyle: React.CSSProperties = { border: '0.5px solid #000', padding: '6px 8px', fontSize: 12, verticalAlign: 'middle' };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 15B — Autres fonds propres</div>
      ...
      {previewUrl && (
        <div className="etat-preview-overlay" onClick={closePreview}>
          <div className="etat-preview-modal" onClick={e => e.stopPropagation()}>
            ...
            <iframe src={previewUrl} className="etat-preview-iframe" title="Aperçu Note 15B" />
          </div>
        </div>
      )}
      ...
      <div ref={pageRef} style={{
        ...
        <div className="etat-header-officiel">
          ...
          <div className="etat-sub-titre">NOTE 15 B<br />AUTRES FONDS PROPRES <sup>(1)</sup></div>
        </div>

        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 0 }}>
          <thead>
            <tr>
              <th style={{ ...thStyle, width: '35%' }}>Libellés</th>
