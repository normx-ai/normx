...
interface Note10Props extends EtatBaseProps {
  onGoToParametres?: () => void;
}

interface RubriqueValeur {
  label: string;
  prefixes: string[];
}

const RUBRIQUES_BRUT: RubriqueValeur[] = [
  { label: 'Effets à encaisser', prefixes: ['511'] },
  { label: 'Effets à l\'encaissement', prefixes: ['512'] },
...
const RUBRIQUES_DEPRECIATION: RubriqueValeur[] = [
  { label: 'Dépréciations des valeurs à encaisser', prefixes: ['591'] },
];

function Note10({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note10Props): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    fetch('/api/entites/' + entiteId)
      ...
      .then(ent => {
        const data = ent.data || {};
        setParams(data);
        setCommentaire(data['note10_commentaire'] || DEFAULT_COMMENTAIRE);
        if (data['note10_adjustments']) {
          try { setAdjustments(JSON.parse(data['note10_adjustments'])); } catch { /* */ }
        }
      })
      .catch(() => {});
  }, [entiteId]);
  ...
  const handleSave = async () => {
    ...
    try {
      const data: Record<string, string> = {
        ...params,
        note10_adjustments: JSON.stringify(adjustments),
        note10_commentaire: commentaire,
      };
  ...
  const computeRow = (r: RubriqueValeur, crediteur = false) => {
  ...
  const downloadPDF = () => { if (!pdfBlob) return; const url = URL.createObjectURL(pdfBlob); const a = document.createElement('a'); a.href = url; a.download = 'Note10_' + annee + '.pdf'; a.click(); URL.revokeObjectURL(url); };
  ...
  return (
    <div>
      <div className="etat-toolbar">
        ...
        <div className="etat-toolbar-title">Note 10 — Valeurs à encaisser</div>
      ...
      {previewUrl && (
        <div className="etat-preview-overlay" onClick={closePreview}>
          <div className="etat-preview-modal" onClick={e => e.stopPropagation()}>
            ...
            <iframe src={previewUrl} className="etat-preview-iframe" title="Aperçu Note 10" />
          </div>
        </div>
      )}
      ...
      <BalanceSourcePanel
        ...
        title="Soldes balance — Valeurs a encaisser"
      />
      ...
      <div ref={pageRef} style={{
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 0 }}>
          ...
          <tbody>
            ...
            {renderTotalRow('TOTAL BRUT VALEURS A ENCAISSER', totalBrut, totalBrutVar)}
