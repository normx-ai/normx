...
function Note10({ entiteName, entiteNif = '', entiteId, offre, onBack }: Note10Props): React.JSX.Element {
  ...
  const pdf = usePDFPreview({ pageRef, fileName: `Note10_${annee}.pdf`, editing, setEditing, orientation: 'l' });
  ...
  useEffect(() => {
    if (!params['note10_commentaire'] && !params['note10_adjustments']) return;
    setCommentaire(params['note10_commentaire'] || DEFAULT_COMMENTAIRE);
    if (params['note10_adjustments']) {
      try { setAdjustments(JSON.parse(params['note10_adjustments'])); } catch { /* */ }
    }
  }, [params]);
