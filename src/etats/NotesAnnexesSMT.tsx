...
function NotesAnnexesSMT({ entiteName, entiteNif = '', entiteId, onBack }: NotesAnnexesSMTProps): React.JSX.Element {
  ...
  useEffect(() => { if (!entiteId) return; clientFetch(api.entites.byId(entiteId)).then(r => r.json()).then(ent => { const d = ent.data || {}; setParams(d); if (d['smt_materiel']) { try { const p = JSON.parse(d['smt_materiel']); if (Array.isArray(p) && p.length > 0) setMateriel(p); } catch { /* */ } } if (d['smt_stocks']) { try { const p = JSON.parse(d['smt_stocks']); if (Array.isArray(p) && p.length > 0) setStocks(p); } catch { /* */ } } if (d['smt_stock_final']) setStockFinal(d['smt_stock_final']); if (d['smt_stock_initial']) setStockInitial(d['smt_stock_initial']); if (d['smt_creances']) { try { const p = JSON.parse(d['smt_creances']); if (Array.isArray(p) && p.length > 0) setCreances(p); } catch { /* */ } } if (d['smt_dettes']) { try { const p = JSON.parse(d['smt_dettes']); if (Array.isArray(p) && p.length > 0) setDettes(p); } catch { /* */ } } }).catch(() => {}); }, [entiteId]);
  const handleSave = async () => { setSaving(true); try { const d: Record<string, string> = { ...params, smt_materiel: JSON.stringify(materiel), smt_stocks: JSON.stringify(stocks), smt_stock_final: stockFinal, smt_stock_initial: stockInitial, smt_creances: JSON.stringify(creances), smt_dettes: JSON.stringify(dettes) }; const r = await clientFetch(api.entites.byId(entiteId), { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data: d }) }); if (r.ok) { setParams(d); setEditing(false); } } catch { /* */ } setSaving(false); };
