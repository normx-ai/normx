...
function BalanceRevisee({ entiteId, exerciceId, exerciceAnnee }: BalanceReviseeProps): React.ReactElement {
  ...
  useEffect(() => {
    ...
    Promise.all([
      clientFetch(api.balance.byExercice(entiteId, exerciceId, 'N')).then(r => r.json()),
      clientFetch(api.revision.onglet(entiteId, exerciceId, 'all-od')).then(r => r.ok ? r.json() : { odEcritures: [] }),
    ]).then(([balData, odData]) => {
