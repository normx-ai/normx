...
router.post('/register', async (req, res) => {
  ...
  if (Array.isArray(reqModules) && reqModules.length > 0) {
    modules = reqModules.filter(m => validModules.includes(m));
  ...
  try {
    ...
    const entiteRow = entiteResult.rows[0];
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const entites = entitesResult.rows;
...
router.get('/entites/cabinet/:cabinetId', async (req, res) => {
...
router.post('/entites', async (req, res) => {
  ...
  const mods = modules || ['compta', 'etats', 'paie'];
...
// Modifier une entite
router.put('/entites/:id', async (req, res) => {
  const { id } = req.params;
...
router.delete('/entites/:id', async (req, res) => {
  const { id } = req.params;
  try {
